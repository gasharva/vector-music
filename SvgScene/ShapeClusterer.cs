...
public sealed class ShapeClusterer : IShapeClusterer
{
    ...
    public ShapeClusterer(IGeometryAnalyzer? geometryAnalyzer = null, IArcExtractor? arcExtractor = null, double distanceThreshold = 0.035)
    {
    ...
    public NotationScene Cluster(GeometricScene scene)
    {
        ...
        var prototypes = new List<ShapePrototype>(); var instances = new List<ShapeInstance>();
        var strokes = new List<Stroke>(); var arcs = new List<Arc>();
        foreach (var shape in scene.Shapes)
        {
            if (_geometryAnalyzer.TryCreateStroke(shape, out var stroke)) { strokes.Add(stroke); continue; }
            if (_arcExtractor.TryCreateArc(shape, out var arc)) { arcs.Add(arc); continue; }
